...
public class Controlador extends HttpServlet {
    ...
    protected void edit(HttpServletRequest request, HttpServletResponse response)
     ...
     protected void inicio(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String rut = request.getParameter("txt1");
        String clave = request.getParameter("txt2");
        
        persona n = servicio.iniciarSesion(rut, clave);
         if (n != null) {
             request.getSession().setAttribute("person", n);
             response.sendRedirect("index.jsp");
         } else {
             request.setAttribute("msg", "Rut no encontrado o clave incorrecta");
